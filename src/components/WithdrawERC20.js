...
class WithdrawERC20 extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      amount: '',
  ...
  async web3Lookup() {
    ...
    if (this.state.loading) {
      let mainnetChain = new MainnetChain(this.props.mainnetWeb3, proxyMainnet);

      let erc20OnMainnet = tokensMeta.erc20[this.state.tokenSymbol].address;
      let erc20Contract = new mainnetChain.web3.eth.Contract(erc20Abi.abi, erc20OnMainnet);
      mainnetChain.addERC20Token(this.state.tokenSymbol, erc20Contract);
      ...
      this.state.sChain.addERC20Token(this.state.tokenSymbol, erc20SchainContract);
      ...
      return (
        <div className="fullscreen-msg">
            <div className="flex-container">
              ...
              <div className="flex-container fl-centered">
                <h3 className='fullscreen-msg-text'>
                  Withdrawing {this.state.amount} {this.state.tokenSymbol} (ERC20) from {this.props.currentSchain}
                </h3>
              </div>
            </div>
        </div>
          ...
          <Card>
            <CardContent>
              <h2 className='card-header'>
                Withdraw {this.state.tokenSymbol} (ERC20)
              </h2>
...
export default withRouter(WithdrawERC20);